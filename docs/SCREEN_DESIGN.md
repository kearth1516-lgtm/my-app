...
- `POST /api/timers/:id/stop?tag=xxx`: タイマー停止（記録保存、タグはオプション）
...
- `POST /api/records/manual`: 手動記録作成
- `PUT /api/records/:id`: 記録更新（duration, date, tag）
...
- **データベース**: Azure Cosmos DB for NoSQL（サーバーレスモード）
  ...
  - データベース名: my-app-db
  - コンテナ: timers, tags, settings, records（パーティションキー: /id）
  - リージョン: Japan East
