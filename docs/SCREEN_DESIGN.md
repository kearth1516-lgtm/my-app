...
- **日時・天気セクション（2カラムグリッド）**
  - **日時カード**:
    ...
    - 時刻表示: HH:MM（1分ごと自動更新）
  - **天気カード**:
    - 天気アイコン（OpenWeatherMap互換）
    - 気温（°C）
    - 天気説明（日本語）
    - 風速（m/s）※湿度は表示されない場合あり
...
#### 天気情報
- **データソース**: Open-Meteo API（完全無料、APIキー不要）
  - エンドポイント: https://api.open-meteo.com/v1/forecast
  - パラメータ: latitude（緯度）、longitude（経度）、current_weather=true
  - レスポンス: temperature, weathercode, windspeed
  - WMO Weather interpretation codes（天気コード 0-99）を日本語に変換
  - OpenWeatherMap互換のアイコンコード生成

- **表示内容**:
  - 気温: 小数点以下切り捨て
  - 天気: WMOコードから日本語変換（例: 0→快晴, 61→小雨, 95→雷雨）
  - アイコン: OpenWeatherMapのアイコン画像を使用
  - 風速: m/s単位
  - 湿度: Open-Meteo無料版では提供されない（null）
...
#### カレンダー連携
- **データソース**: Googleカレンダー（iCal形式）
  - 秘密のアドレス（private URL）を使用
  - カレンダーを公開する必要なし
  - 設定画面で秘密のアドレス（iCal形式）を設定

- **取得方法**:
  1. Googleカレンダーの「設定と共有」を開く
  2. 「カレンダーの統合」セクションを探す
  3. 「秘密のアドレス（iCal形式）」のURLをコピー
  4. 設定画面に貼り付けて保存
...
- **対応URL形式**:
  - iCal URL: `https://calendar.google.com/calendar/ical/.../private-xxx/basic.ics`
  - cid付きURL: `https://calendar.google.com/calendar/u/1?cid=xxx`（自動変換）
  - メールアドレス形式: `example@gmail.com`（公開カレンダーのみ）

#### スタイル
- 背景: テーマカラーのグラデーション（`var(--theme-gradient)`）
- 文字色: テーマに応じた色（`var(--theme-text-color)`）
- 日時・天気カード: 白背景、角丸15px、シャドウ付き
- スケジュールカード: 白背景、角丸10px、左側に時刻表示
- ホバー効果: 上方向に5px移動

#### 機能
- リアルタイム時計: 1分ごとに自動更新
- 天気情報: ページ読み込み時に自動取得
- スケジュール: ページ読み込み時に自動取得
- ランダム画像: ページ読み込み時に取得
- 設定ボタン: 設定画面へ遷移
- メニューアイテム: 各画面へ遷移
...
- **AI機能設定セクション**
  - セクションタイトル: "🤖 AI機能設定"
  - 説明文: "OpenAI APIキーを設定するとAI提案機能が使えます"
  - OpenAI API Key入力欄:
    - パスワード形式（👁️/🔒で表示切り替え）
    - プレースホルダー: "sk-..."
  - 保存ボタン
  - 注意事項: APIキー取得方法のリンク付き

- **カレンダー連携セクション**
  - セクションタイトル: "📅 カレンダー連携"
  - 説明文: "Googleカレンダーの秘密のアドレス（iCal形式）を設定"
  - カレンダーURL入力欄:
    - テキスト形式
    - プレースホルダー: "https://calendar.google.com/calendar/ical/.../private-xxx/basic.ics"
  - 保存ボタン
  - 取得方法の詳細手順:
    1. Googleカレンダーを開く（PCブラウザで）
    2. 左側の「マイカレンダー」で連携したいカレンダーにカーソルを合わせる
    3. 「︙」（縦3点メニュー）→「設定と共有」をクリック
    4. 下にスクロールして「カレンダーの統合」セクションを探す
    5. 「秘密のアドレス（iCal 形式）」のURLをコピー
    6. 上の入力欄に貼り付けて保存
  - メリット: カレンダーを公開設定にする必要なし
  - 注意: このURLを知っている人は誰でもカレンダーを見れます
